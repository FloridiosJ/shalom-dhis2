...
describe('Reports Service', () => {
  ...
  describe('exportReport', () => {
    it('should export report in CSV format', async () => {
        ...
        data: {
          data: {
            exportReport: {
              ...
              url: 'http://localhost:4000/download/report-123.csv',
      ...
      const result = await reportsService.exportReport('csv', filters);
      
      expect(mockPost).toHaveBeenCalledWith('', {
        query: expect.stringContaining('ExportReport'),
        variables: { format: 'csv', filters }
      });
      expect(result.success).toBe(true);
      expect(result.fileName).toBe('report-123.csv');
    ...
    it('should export report in PDF format', async () => {
        ...
        data: {
          data: {
            exportReport: {
              ...
              url: 'http://localhost:4000/download/report-456.pdf',
              fileName: 'report-456.pdf'
            }
          }
        }
      };
      ...
      const result = await reportsService.exportReport('pdf', filters);
      
      expect(mockPost).toHaveBeenCalledWith('', {
        query: expect.any(String),
        variables: { format: 'pdf', filters }
      });
      expect(result.success).toBe(true);
    ...
    it('should handle export failures', async () => {
      ...
      const filters = { dispensaireId: 'invalid' };
      const result = await reportsService.exportReport('csv', filters);
      
      expect(result.success).toBe(false);
      expect(result.message).toContain('Aucune donnée disponible');
    });

    it('should handle GraphQL errors during export', async () => {
      const mockResponse = {
        data: {
          errors: [{ message: 'Format non supporté' }]
        }
      };
      ...
      const filters = {};
      await expect(
        reportsService.exportReport('invalid', filters)
