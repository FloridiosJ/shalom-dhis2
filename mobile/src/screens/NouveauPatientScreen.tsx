...
export default function NouveauPatientScreen({
  navigation,
}: NouveauPatientScreenProps) {
  const [isSubmitting, setIsSubmitting] = useState(false);
  const [showReligionPicker, setShowReligionPicker] = useState(false);

  const [createPatient] = useMutation<CreatePatientResponse>(
    CREATE_PATIENT_MUTATION,
  );
  ...
  const onSubmit = async (data: PatientFormData) => {
    try {
      ...
      if (!user?.dispensaireId) {
        Alert.alert(
          'Erreur',
          'Dispensaire non configuré. Veuillez vous reconnecter.',
        );
        return;
      }
